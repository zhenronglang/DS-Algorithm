...
do
	...
	do
		...
		do
			make clean
			nvcc -O3 -arch=sm_52 unique.cu -I/usr/local/cuda/include -lm -o unique -D atom -D COARSENING=$co -D THREADS=$th -D INT  	
			...
			echo "$th, $co, $i, $Throughput" >> uniqueatomic.csv
		done
		echo " " >> uniqueatomic.csv
	done
done
