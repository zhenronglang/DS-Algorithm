...
int main(int argc, char **argv){
  ...
  printf("DS Stream Compaction on this %s\n", device_properties.name);
...
//#define REP 4
    ...
    //Thrust 
    thrust::device_vector<int> d_src = h_A;

    //Result vector
    thrust::device_vector<int> d_res(d_src.size());

    //Copy compacted elements from d_src to d_res
    auto result_end = thrust::remove(d_src.begin(), d_src.end(), d_res.begin(), value);

    //Copy back to host

    thrust::host_vector<int> h_B(d_res.begin(), result_end);
